...
df_daily = df.groupby(by=df['pickup_datetime'].dt.date).sum()


###plot
def plot_day(x,date_str):
    x = x[x['date']==date_str]
    x['time'] = x['pickup_datetime'].dt.time
    x['time'] = x['time'].apply(lambda x: str(x))
    x = x.set_index('time').sort_index()
    plt.plot(x['passenger_count'])
    plt.xticks(rotation=45)


plot_day(df,'2020-11-01')
#
